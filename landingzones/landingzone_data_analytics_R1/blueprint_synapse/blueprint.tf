...
resource "azurerm_resource_group" "rg_synapse_anaytics" {
...
module "synapse_storage" {
  ...
  prefix                   = var.prefix
  convention               = var.convention
  resource_group_name      = azurerm_resource_group.rg_synapse_anaytics.name
  location                 = var.location
...
resource "azurerm_template_deployment" "synapse_workspace" {
  ...
  depends_on = [
    azurerm_resource_group.rg_synapse_anaytics,
    module.synapse_storage,
    var.vnet,
